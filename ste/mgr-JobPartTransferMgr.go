...
type IJobPartTransferMgr interface {
	...
	ReportTransferDoneWithError(errMsg string) uint32
	RescheduleTransfer()
...
func (jptm *jobPartTransferMgr) Info() TransferInfo {
	...
	//jptm.transferInfo.Source = "https://dmo0001.oss-eu-central-1.aliyuncs.com/icudtl.dat"
	//jptm.transferInfo.Destination = "https://s32blobdestination01.blob.core.windows.net/202410291/icudtl.dat"
...
func (jptm *jobPartTransferMgr) FailActiveS2SCopy(where string, err error) {
	glog.Errorf("Error During COPY %s : %s", where, err)
...
func (jptm *jobPartTransferMgr) FailActiveSend(where string, err error) {
	...
	if isUpload {
		...
		if err != nil {
			...
			serviceCode, _, msg := ErrorEx{err}.ErrorCodeAndString()

			glog.Debugf("%s Fail Active Send Error: %s, message: %s, job: %s", where, serviceCode, msg, jptm.transferInfo.Source)

			// if serviceCode == "" {
			// 	errNode := pipeline.ErrorNode{}.Initialize(err, 3)

			// 	s1 := errNode.Cause().Error()
			// 	s2 := errNode.Unwrap().Error()
			// 	if s1 != s2 {
			// 		glog.Debugf("%s Fail Active Send Error: %s, message: %s, job: %s", where, serviceCode, msg, jptm.transferInfo.Source)
			// 	}

			// 	errStr := err.Error()
			// 	if strings.Contains(errStr, "The specified resource does not exist") {
			// 		serviceCode = "BlobNotFound"
			// 	} else if strings.Contains(errStr, "The specified blob does not exist") {
			// 		serviceCode = "BlobNotFound"
			// 	} else if strings.Contains(errStr, "The specified container does not exist") {
			// 		serviceCode = "ContainerNotFound"
			// 	} else if strings.Contains(errStr, "The specified share does not exist") {
			// 	}
			// }
			common.GetLifecycleMgr().SetSingleJobDataToEventhubB2B(
				strings.TrimPrefix(jptm.transferInfo.Source, "/"),
				common.Job_Status_Failed,
				fmt.Sprintf("%s:%s", serviceCode, msg),
				common.Job_Status_Failed,
			)
		}
	} else {
		...
		// transfer
		jptm.FailActiveDownload(fmt.Sprintf("%s check operation type, is it really download?", where), err)
	}
}
...
func (jptm *jobPartTransferMgr) Log(level pipeline.LogLevel, msg string) {
	...
	if common.LogLevel(level).String() == "ERR" {
		glog.Errorf("Error : %s", fmt.Sprintf("%s: [P#%d-T#%d] ", common.LogLevel(level), plan.PartNum, jptm.transferIndex)+msg)
	}
...
func (jptm *jobPartTransferMgr) LogS2SCopyError(source, destination, errorMsg string, status int) {
	glog.Errorf("Error During COPY From %s to %s : %s", source, destination, errorMsg)
...
func (jptm *jobPartTransferMgr) ReportTransferDone() uint32 {
	...
	//TODO DELETEME
	//glog.Infof("Enter Report Job Done, Job %s Status %d", jptm.Info().Source, jptm.jobPartPlanTransfer.TransferStatus())

	statusCode := common.Job_Status_Failed
	messageString := "Job Failed"
	fileName, _ := url.Parse(jptm.Info().Source)
	if jptm.jobPartPlanTransfer.TransferStatus() == -3 {
		statusCode = common.Job_Status_Success
		messageString = "File skipped, already exist"
	}
	if jptm.jobPartPlanTransfer.TransferStatus() == 2 {
		statusCode = common.Job_Status_Success
		messageString = "File transferred successes or skipped, already exist"
	}
	if jptm.jobPartPlanTransfer.atomicErrorCode == 404 {

		statusCode = common.Job_Status_Failed
		messageString = common.Err_Message_404
	}
	if jptm.jobPartPlanTransfer.atomicErrorCode == 403 {

		statusCode = common.Job_Status_Failed
		messageString = common.Err_Message_403
	}

	//for del

	if jptm.jobPartPlanTransfer.TransferStatus() == 2 && os.Getenv(util.TaskTypeDeltaDelete) == util.TaskTypeDeltaDelete {
		statusCode = common.Job_Status_Success
		messageString = "File Deleted successes"

	}
	//620 common.GetLifecycleMgr().SetSingleJobDataToCosmos(strings.TrimPrefix(fileName.Path, "/"), statusCode, messageString, int(jptm.jobPartPlanTransfer.TransferStatus()))
	//630 common.GetLifecycleMgr().SetSingleJobDataToEventhub(strings.TrimPrefix(fileName.Path, "/"), statusCode, messageString, int(jptm.jobPartPlanTransfer.TransferStatus()))
	common.GetLifecycleMgr().SetSingleJobDataToEventhubB2B(strings.TrimPrefix(fileName.Path, "/"), statusCode, messageString, int(jptm.jobPartPlanTransfer.TransferStatus()))
	...
	if atomic.SwapUint32(&jptm.atomicCompletionIndicator, 1) != 0 {
		...
		glog.Errorf("cannot report the same transfer done twice: %s\n", fileName.Path)
	}
...
func (jptm *jobPartTransferMgr) ReportTransferDoneWithError(errMsg string) uint32 {
	// In case of context leak in job part transfer manager.
	jptm.Cancel()
	...
	statusCode := common.Job_Status_Failed
	messageString := fmt.Sprintf("Job Failed: %s", errMsg)
	fileName, _ := url.Parse(jptm.Info().Source)
	if jptm.jobPartPlanTransfer.TransferStatus() == -3 {
		statusCode = common.Job_Status_Success
		messageString = "File skipped, already exist"
