...
// defaultGCPPresignExpires is set to 7 days, which is the maximum allowed for GCP V4 signed URLs.
// This ensures that for huge files, the source URL remains valid for the duration of the copy.
const defaultGCPPresignExpires = time.Hour * 24 * 7
...
func newGCPSourceInfoProvider(jptm IJobPartTransferMgr) (ISourceInfoProvider, error) {
	...
	// */
	// credPath, err := glcm.EnsureGCPServiceAccountFile()
	// if err != nil {
	// 	return nil, err
	// }
	// jsonKey, err = os.ReadFile(credPath)
	// if err != nil {
	// 	return nil, fmt.Errorf("Cannot read JSON key file from Key Vault/Redis cache: %v", err)
	// }
	return &p, nil
}
...
func (p *gcpSourceInfoProvider) PreSignedSourceURL() (*url.URL, error) {
	...
	opts := &gcpUtils.SignedURLOptions{
		...
		Expires:        time.Now().Add(defaultGCPPresignExpires),
	}
