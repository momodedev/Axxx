...
func (t *gcpTraverser) Traverse(preprocessor objectMorpher, processor objectProcessor, filters []ObjectFilter) error {
	...
	for {
		...
		if err == nil {
			...
			// The iterator already returns the object attributes, so we don't need to fetch them again.
			// This saves a network call for every object, significantly improving enumeration performance for large buckets.
			oie := common.GCPObjectInfoExtension{ObjectInfo: *attrs}
